...
class Agent:
    ...
    def make_one_transition(self, action):
        ...
        next_r = self.simulator.check_reward()
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            trajectory.append((s, self.simulator.check_reward()))

            while not self.simulator.game_over():
                a = self.default_policy(s)
                s, r = self.make_one_transition(a)
        ...
        for simulation in range(num_simulation):
            ...
            while s is not None:
                ...
                if next_s is not None:
                    err = r + DISCOUNT * self.TD_values[next_s] - self.TD_values[s]
                    self.TD_values[s] += self.alpha(self.N_TD[s]) * err
                else:
                    self.TD_values[s] = r
                self.N_TD[s] += 1
                s = next_s
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while s is not None:
                ...
                next_s, next_r = self.make_one_transition(a)
                if next_s is not None:
                    err = r + DISCOUNT * max(self.Q_values[next_s][HIT], self.Q_values[next_s][STAND]) - self.Q_values[s][a]
                    self.Q_values[s][a] += self.alpha(self.N_Q[s][a]) * err
                else:
                    self.Q_values[s][a] = r
                self.N_Q[s][a] += 1
                s = next_s
    ...
    def pick_action(self, s, epsilon):
        ...
        if random.randint(0, 1) < epsilon:
            ...
            return STAND if self.Q_values[s][STAND] > self.Q_values[s][HIT] else HIT
